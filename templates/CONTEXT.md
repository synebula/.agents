...
- **[模块 1 路径]/**：[1 句话说明职责] → 详见 `[模块1路径]/.agents/CONTEXT.md`
- **[模块 2 路径]/**：[1 句话说明职责] → 详见 `[模块2路径]/.agents/CONTEXT.md`
...
> 只记录“现在仍然有效”的约定/选择；不要写操作历史/时间线。过程与证据写入 `.agents/TASK-*.md`；如维护全局历史再写 `.agents/CHANGELOG.md`。

- **[主题 1]**：[当前约定/选择]（涉及：`路径/模块`）
- **[主题 2]**：[当前约定/选择]（涉及：`路径/模块`）
...
4. **不记历史**：CONTEXT.md 只维护现状快照，不记录操作历史/过程日志/验证结果（过程与证据写入 TASK/CHANGELOG）
